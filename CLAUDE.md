...
`detectFaces()` — multi-face, **cluster-first, then orientation**. The visible faces of an angled cube touch each other, so proximity clustering merges them into one dense blob — which is exactly what we want as a *first* step: `clusterStickers()` isolates that cube blob and drops background false-positives (t-shirt folds, bedsheet creases, tiles, a stray finger highlight) into their own tiny clusters. Only *then* do we split each cluster into faces by edge orientation: every sticker on a given face is a parallelogram whose two edge directions are shared across that face (the 2D projection of the face's grid axes), and an adjacent face projects a *different* pair, so `groupByOrientation()` separates the touching faces by their unordered edge-angle pair (needs only ≥3 co-oriented stickers per face). Clustering before the orientation split matters: a stray background square that happens to share a face's edge-orientation pair would otherwise pollute that face's group and drag the grid fit off the cube (the failure mode on hand-held shots with a textured background). Within each face group `fitGrid()` reconstructs the 3×3 — a full 9-sticker face (sort-by-v group-by-3) or a partial 5–8 face (oblique-basis decomposition + 1D k-means, tolerant to an occluded sticker; it still needs ≥3 distinct columns *and* rows, so a face whose entire third column is occluded/undetected can't be fit).
...
**Known limit (occluded hand-held angle):** an angled hand-held cube where fingers occlude the front-bottom and the foreshortened side faces have white stickers (e.g. the `spin6063964` fixture) only yields the fully-visible **top** face. Cluster-first grouping now isolates the cube and fits that top face correctly (the previous orientation-first pass let background squares pollute the groups and returned two garbage faces), but the left/right faces lose a whole column to occlusion + un-contourable white stickers, so `fitGrid` can't reconstruct them. Recovering them needs the side faces unoccluded, less foreshortening, or a learned model — not classical contour detection.
